...
PQC_ALGORITHMS: dict = {
    ...
    "SLH-DSA-SHA2-128f": {
        ...
        "oqs_names": ("SLH-DSA-SHA2-128f", "SLH_DSA_PURE_SHA2_128F", "SPHINCS+-SHA2-128f-simple"),
    ...
    "SLH-DSA-SHA2-128s": {
        ...
        "type": "hash", "family": "SLH-DSA", "nist_level": 1,
        "fips": "FIPS 205", "experimental": False,
        "oqs_names": ("SLH-DSA-SHA2-128s", "SLH_DSA_PURE_SHA2_128S", "SPHINCS+-SHA2-128s-simple"),
    ...
    "SLH-DSA-SHA2-192f": {
        ...
        "fips": "FIPS 205", "experimental": False,
        "oqs_names": ("SLH-DSA-SHA2-192f", "SLH_DSA_PURE_SHA2_192F", "SPHINCS+-SHA2-192f-simple"),
    ...
    "SLH-DSA-SHA2-256f": {
        ...
        "fips": "FIPS 205", "experimental": False,
        "oqs_names": ("SLH-DSA-SHA2-256f", "SLH_DSA_PURE_SHA2_256F", "SPHINCS+-SHA2-256f-simple"),
    ...
    "SLH-DSA-SHAKE-128f": {
        ...
        "type": "hash", "family": "SLH-DSA", "nist_level": 1,
        "fips": "FIPS 205", "experimental": False,
        "oqs_names": ("SLH-DSA-SHAKE-128f", "SLH_DSA_PURE_SHAKE_128F", "SPHINCS+-SHAKE-128f-simple"),
    ...
    "SLH-DSA-SHAKE-256f": {
        ...
        "type": "hash", "family": "SLH-DSA", "nist_level": 5,
        "fips": "FIPS 205", "experimental": False,
        "oqs_names": ("SLH-DSA-SHAKE-256f", "SLH_DSA_PURE_SHAKE_256F", "SPHINCS+-SHAKE-256f-simple"),
...
def _subject_extensions_der(
    subject_spki: bytes,
    ca_ski: Optional[bytes],
    *,
    is_ca: bool,
    ...
    """Construit le bloc Extensions ([3] EXPLICIT) commun aux certificats émis
    par le moteur DER (sujet PQC). Extensions RFC 5280 : BasicConstraints,
    KeyUsage, EKU (end-entity), SAN, SKI, AKI, AIA, CRLDP.
    """
    extensions = []

    # BasicConstraints (critique)
    if is_ca:
        ...
        bc = _der_seq()  # cA = FALSE par défaut
    ...
    # ExtendedKeyUsage (serverAuth + clientAuth) — end-entity uniquement
    if not is_ca:
    ...
    extensions.append(_extension_der("2.5.29.14", False, _der_octet(_ski_from_spki(subject_spki))))

    # AuthorityKeyIdentifier
    if ca_ski is not None:
        extensions.append(
            _extension_der("2.5.29.35", False, _der_seq(_der_ctx(0, ca_ski, constructed=False)))
        )
    ...
    if crl_url:
        ...
        full_name = _der_ctx(0, gn)
        dist_point = _der_ctx(0, full_name)
        extensions.append(_extension_der("2.5.29.31", False, _der_seq(_der_seq(dist_point))))

    return _der_ctx(3, _der_seq(*extensions))


# OID des algorithmes de signature classiques (RFC 5280 §A.2, RFC 8410).
_RSA_SIG_OID = {
...
_ED448_OID = "1.3.101.113"


def _ca_signer_and_algid(ca_private_key_pem, ca_key_password, signature_hash):
    """Résout la clé privée de la CA (PQC ou classique) en un signataire du TBS.

    :returns: tuple (sign_callable(tbs)->bytes, algid_der, ca_algo_label).
    """
    # 1) Clé PQC (PKCS#8 maison ou conteneur chiffré) ?
    try:
        alg, raw_secret = load_pqc_private_key_pem(ca_private_key_pem, ca_key_password)
        signer = PQCSigner(alg)
        oid = PQC_ALGORITHMS[alg]["oid"]
        return (lambda tbs: signer.sign(tbs, raw_secret)), _algid(oid), alg
    except PrivateKeyLoadError:
        pass  # pas une clé PQC → on tente le chargement classique

    # 2) Clé classique via cryptography
    ...
    h = (signature_hash or "sha256").strip().lower()
    hash_obj = {"sha256": hashes.SHA256(), "sha384": hashes.SHA384(),
                "sha512": hashes.SHA512()}.get(h, hashes.SHA256())

    if isinstance(key, rsa.RSAPrivateKey):
        oid = _RSA_SIG_OID.get(h, _RSA_SIG_OID["sha256"])
        # RSA : le champ parameters de l'AlgorithmIdentifier DOIT être NULL.
        algid = _algid(oid, params=_der_null())
        return (lambda tbs: key.sign(tbs, padding.PKCS1v15(), hash_obj)), algid, "RSA"
    if isinstance(key, ec.EllipticCurvePrivateKey):
        oid = _ECDSA_SIG_OID.get(h, _ECDSA_SIG_OID["sha256"])
        return (lambda tbs: key.sign(tbs, ec.ECDSA(hash_obj))), _algid(oid), "EC"
    if isinstance(key, Ed25519PrivateKey):
        return (lambda tbs: key.sign(tbs)), _algid(_ED25519_OID), "Ed25519"
    if isinstance(key, Ed448PrivateKey):
        return (lambda tbs: key.sign(tbs)), _algid(_ED448_OID), "Ed448"
    raise UnsupportedAlgorithmError(
        "Type de clé CA non supporté pour la signature (attendu RSA/EC/Ed25519/Ed448/PQC)."
    )


def createSignedCertHybrid(
    ...
    """Émet un certificat X.509 dont le **sujet** porte une clé post-quantique
    (ML-DSA / Falcon / SLH-DSA), signé par une CA dont la clé peut être
    **classique** (RSA / EC / Ed25519 / Ed448) **ou** PQC.

    Indispensable car `cryptography` refuse une clé publique PQC comme sujet de
    `CertificateBuilder` : on encode donc le TBSCertificate en DER puis on le
    signe avec la clé de la CA (algorithme de signature = celui de la CA, la
    clé du sujet restant indépendante — RFC 5280). L'`issuer` et
    l'AuthorityKeyIdentifier sont dérivés de `ca_cert_pem`.

    Pour un sujet classique signé par une CA classique, utiliser plutôt
    `createSignedCertFromBytes` (chemin cryptography natif).
    """
    ...
    issuer_name = ca_cert.subject.public_bytes()  # Name DER de l'émetteur

    # AuthorityKeyIdentifier : keyIdentifier de la CA. On lit son extension SKI ;
    # à défaut on le calcule depuis sa clé publique (CA classique).
    ca_ski = None
    try:
        ...
        try:
            from cryptography.hazmat.primitives import serialization
            ca_spki_der = ca_cert.public_key().public_bytes(
                encoding=serialization.Encoding.DER,
                format=serialization.PublicFormat.SubjectPublicKeyInfo,
            )
            ca_ski = _ski_from_spki(ca_spki_der)
        except Exception:
            ca_ski = None
    ...
    extensions_der = _subject_extensions_der(
        subject_spki, ca_ski, is_ca=is_ca, path_length=path_length, email=email,
        alt_names=alt_names, ip_addresses=ip_addresses, ocsp_url=ocsp_url,
        ca_issuers_url=ca_issuers_url, crl_url=crl_url,
    )

    sign_callable, sig_algid, _ca_label = _ca_signer_and_algid(
        ca_private_key_pem, ca_key_password, signature_hash)
    ...
    try:
        signature = sign_callable(tbs)
    except Exception as e:
        from .exceptions import CertificateSigningError
        raise CertificateSigningError(f"Échec de la signature du certificat hybride : {e}")

    certificate = _der_seq(tbs, sig_algid, _der_bitstring(signature))
...
def createSignedCertPQC(
    ...
    # ── Extensions (bloc partagé avec createSignedCertHybrid) ─────────────────
    ca_ski = _ski_from_spki(ca_spki) if ca_spki is not None else None
    extensions_der = _subject_extensions_der(
        subject_spki, ca_ski, is_ca=is_ca, path_length=path_length, email=email,
        alt_names=alt_names, ip_addresses=ip_addresses, ocsp_url=ocsp_url,
        ca_issuers_url=ca_issuers_url, crl_url=crl_url,
